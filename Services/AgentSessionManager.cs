    ...
    ILogger<AgentSessionManager> logger)
{
    ...
    public IReadOnlyList<AgentWorkspaceSummary> ListWorkspaces(string? scope = null) => workspaces.List(scope);
    ...
    public bool HasActiveWorkspace(string workspaceId)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            logger.LogInformation(
                "Starting Agent session {SessionId} bound to workspace {WorkspaceId} at {WorkspacePath}; Agent working directory is {WorkingDirectory}",
                sessionId,
                workspace.Summary.Id,
                workspace.Path,
                allocation.WorkingDirectory);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            if (session.IsPromptSubmissionInProgress)
            {
                throw new InvalidOperationException("A prompt is already being added to this Agent session.");
            }

            session.IsPromptSubmissionInProgress = true;
        }
        ...
        finally
        {
            lock (session.SyncRoot)
            {
                session.IsPromptSubmissionInProgress = false;
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            PermissionMode = permissionMode ?? summary.PermissionMode,
            Status = "idle"
        });
    ...
    private static string? NormalizeModel(string? model)
    {
        ...
        {
            throw new ArgumentException("Model must not contain control characters.", nameof(model));
        }
        ...
        string workingDirectory)
    {
        ...
        public bool IsPromptSubmissionInProgress { get; set; }
